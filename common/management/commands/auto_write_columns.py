...
3. **## 숫자로 보는 현황** (데이터 근거) — **필수**
   - 주제를 뒷받침하는 **비교 가능한 수치 3~5개**를 제시한다. 예: 연도별 추이(2021·2023·2025),
     집단별 비교(대기업·중견·중소), 항목별 비율(도입 이유 상위 5개)
   - 각 수치마다 어느 기관·보고서의 것인지 본문에 밝힌다 (예: "한국직업능력연구원 2024년 조사에 따르면 …")
   - **널리 공표되어 검증 가능한 통계만** 사용한다. 정확한 값이 기억나지 않으면 그 수치는 쓰지 말고,
     확실히 아는 다른 지표로 대체한다. **절대 지어내지 않는다.**
   - 수치의 의미를 2~3문장으로 해석한다 (무엇이 늘고 줄었는지, 왜 그런지)
   - 이 섹션의 수치는 차트·표로 시각화되므로, 단위(%·명·억 원·점)를 명시한다

4. **## [트렌드 핵심 제목]** (본론)
   - 트렌드의 실체, 핵심 개념, 작동 원리를 깊이 있게 설명
   - `###` 하위 섹션 2~3개로 세분화하여 층위를 나눈다
   - 개념 설명에 그치지 말고 "왜 그렇게 되는지" 메커니즘까지 파고든다
   - 반론이나 한계도 한 단락 다룬다 (무비판적 소개 금지)

5. **## 현장의 변화** (사례 분석)
   - 국내외 조직/기업/학계의 실제 사례 2~3개
   - 사례마다 배경 → 실행 내용 → 결과·한계 순으로 서술
   - 구체적이고 검증 가능한 사례 위주

6. **## 시사점: 우리가 갖춰야 할 것** (실무 적용)
   - 독자 맞춤 실행 가능한 제언 3~5개를 글머리 기호(`-`)로 정리
   - 각 제언은 한 줄 요약 + 2~3문장의 실행 방법으로 구성
...
추가 지침:
- **전체 분량: 본문 기준 2,500~3,500자** (참고 자료 제외, 마크다운 문자 기준).
  분량을 채우려 같은 말을 반복하지 말고, 메커니즘·사례·반론·실행 방법을 더 깊이 파고들어 채운다.
- 한 단락은 3~5문장. 한 문장짜리 단락을 늘어놓지 않는다.
